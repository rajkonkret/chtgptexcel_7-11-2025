...
Public Sub CleanDatesToG()
    ...
    Set ws = ActiveSheet            ' albo ThisWorkbook.Worksheets("Clean Data"), jeœli chcesz na sztywno
    srcCol = 5                      ' E
    dstCol = 7                      ' G
    firstRow = 6
...
Private Function ParsePolishDate(ByVal v As Variant) As Variant
    ...
    ' Uproœæ znaki: ma³e litery, zamieñ polskie znaki, ujednoliæ separatory
    s = LCase$(s)
    s = ReplacePolChars(s)
    ' Miesi¹ce tekstowe -> numery (01–12) z zachowaniem granic wyrazów
    ...
    ' Teraz mamy coœ typu: dd/mm/rrrr, dd/mm/rr albo rrrr/mm/dd
    parts = Split(s, "/")
    If UBound(parts) <> 2 Then Exit Function

    ' Usuñ zera wiod¹ce, ale zabezpiecz przed pustymi
...
Private Function ReplacePolChars(ByVal s As String) As String
    ' Zast¹p polskie znaki bezp³ciowymi odpowiednikami,
    ' bo ktoœ zawsze wpisze "paŸ" jako "paŸ", "paz" albo "PAZ".
    s = Replace(s, "¹", "a")
    s = Replace(s, "æ", "c")
    s = Replace(s, "ê", "e")
    s = Replace(s, "³", "l")
...
Private Function ReplaceMonthWordsWithMM(ByVal s As String) As String
    ' Zamieñ polskie nazwy/skrótowce miesiêcy na numery "01"..."12".
    ' Obs³uguje skróty i pe³ne nazwy, z lub bez kropek.
...
Private Function ReplaceWordToken(ByVal text As String, ByVal token As String, ByVal repl As String) As String
    ' Prosta zamiana „ca³ego s³owa” bez referencji do RegExp.
    ' Dzia³a w najczêstszych uk³adach: oddzielone spacj¹/kropk¹/myœlnikiem/slashem/liczb¹.
